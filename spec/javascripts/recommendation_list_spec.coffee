...
describe 'RecommendationList', ->
  describe 'populate', ->
    ...
    context 'input contains heroes', ->
      ...
      it 'fills first slot', ->
        expect($('.recommend-these .first.character')).to.be.populated_with_hero name: 'Sven', id: 2

      it 'fills second slot', ->
        expect($('.recommend-these .second.character')).to.be.populated_with_hero name: 'Axe', id: 57

      it 'fills third slot', ->
        expect($('.recommend-these .third.character')).to.be.populated_with_hero name: 'Earthshaker', id: 1

      it 'fills fourth slot', ->
        expect($('.recommend-these .fourth.character')).to.be.populated_with_hero name: 'Tiny', id: 3

      it 'fills fifth slot', ->
        expect($('.recommend-these .fifth.character')).to.be.populated_with_hero name: 'Kunkka', id: 4
